...
packages/
  ...
  deployoor-testing/    — @deployoor/testing: createTestClients() (EDR as viem clients + an in-memory store) for node-free tests
...
- **Real-EVM tests via EDR** (`test/evm-clients.ts`'s `makeEvmClients()` → an `@nomicfoundation/edr` provider wrapped in a small EIP-1193 shim and exposed as viem clients over `custom(provider, { retryCount: 0 })`). No fake clients. `makeEvmClients` keeps an **explicit viem return-type annotation** so the emitted declarations stay nameable under `declaration: true` (TS2742). EDR is also what backs `@deployoor/testing`; the two are deliberately the same shape.
