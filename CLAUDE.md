...
packages/
  ...
  deployoor-evm/        — @deployoor/evm: **private, never published.** The in-memory EVM (EDR) as viem clients + cheatcodes. Bundled into @deployoor/testing (tsdown `noExternal`) and imported directly by deployoor's own tests.
  deployoor-testing/    — @deployoor/testing: createTestClients() (@deployoor/evm + an in-memory store) for node-free tests
...
- **Real-EVM tests via `@deployoor/evm`** (`test/evm-clients.ts`'s `makeEvmClients()` is a thin wrapper over `createEvmNode()`). No fake clients. `makeEvmClients` keeps an **explicit viem return-type annotation** so the emitted declarations stay nameable under `declaration: true` (TS2742).
- **Why `@deployoor/evm` exists at all:** `@deployoor/testing` depends on `deployoor`, so `deployoor` cannot depend on it — that closes a cycle turbo rejects outright ("Circular package dependency detected"). Both need the same EVM, so it lives in a third package that depends on neither. It is `private: true` and therefore **inlined** into `@deployoor/testing`'s bundle rather than declared as a runtime dep, which a consumer could not resolve.
- **EDR config has no defaults and several traps.** `createProvider` requires ~20 fields. `hardfork` wants a string (`l1HardforkToString(l1HardforkLatest())`), `MineOrdering` is the enum not the bare string, EIP-7825 caps a transaction at 2^24 gas so a 30M `defaultTransactionGasLimit` fails everything, `ownedAccounts` grants signing but not balance (fund via `genesisState`), and `decodeConsoleLogInputsCallback` belongs to the logger config. **Forking rejects chain genesis state entirely** ("Storage overrides are not supported for forked blocks") — pass only per-account overrides, each with `code: new Uint8Array()`. Hardhat's own source is the only reference for that last one.
- **`@nomicfoundation/edr` must be a `dependency`, never a `devDependency`.** tsdown externalizes deps and peers only, so as a devDep it inlines the seven platform `.node` binaries into `dist/assets` and the tarball goes from 96 KB to 200 MB.
...
- **Node 22+ everywhere in dev.** CI runs Node **22/24**. `@deployoor/testing` declares `engines: ">=22"` because EDR does; the docs site needs 22+ (Vocs 2.x `node:fs/promises` glob); rolldown needs 20+. The core packages' published dist still targets node18, so their `engines: ">=18"` (runtime) stays correct — they are pure JS and nothing about them changed.
...
- **`deployoor`'s tevm devDependency is `@tevm/compiler`, not `tevm`.** The optional peer users install is `@tevm/compiler`, so the devDep mirrors it. Depending on the `tevm` metapackage instead drags in ~48 `@tevm/*` packages (cli, ts-plugin, every bundler plugin) plus `@evmts/zevm@0.0.0`, whose 11 platform `optionalDependencies` are only 4 published — which aborts a Yarn Berry install outright. With just the compiler the tree is 7 packages and no zevm, and needs no `overrides` to resolve cleanly. Don't "simplify" it back to `tevm`.
- **Overrides live in `pnpm-workspace.yaml`, not `package.json`'s `pnpm` block.** When both exist the `package.json` one wins silently and the workspace file is ignored with no warning.
- **`pnpm add` rewrites `pnpm-workspace.yaml` and eats it.** Adding a dependency that touches the catalog makes pnpm reserialize the whole file: every comment is deleted, quoting flips to single quotes so `format:check` fails, and a catalog range is replaced by the exact version it just resolved. That last one is the dangerous half — one `pnpm add -D effect` in `evals/` silently turned the engine's `effect: ^3.19.0` into `effect: 3.21.4`, because `packages/deployoor` declares `effect: "catalog:"`. The comments it deleted were the ones explaining the viem 2.46.0 cap, the overrides placement, and the `@tevm/solc` extension, all of which exist precisely because they are not guessable. After any `pnpm add`, diff this file before committing.
- **`@tevm/solc` needs a `packageExtensions` entry for `solc`.** It calls `require("solc")` but declares it nowhere — not a dependency, not a peer. It only ever resolved because the `tevm` metapackage pulled solc in alongside it; once deployoor depends on `@tevm/compiler` directly, pnpm's isolated layout correctly refuses to resolve it and `examples/tevm` fails with `Cannot find module 'solc'`. The entry in `pnpm-workspace.yaml` declares it on their behalf.
...
- **The docs site's machine-readable surface has five moving parts, none of them a Vocs feature.** Vocs generates `llms.txt`/`llms-full.txt` from the page tree with no hook for extra content, so `apps/docs/scripts/augment-llms.mjs` runs after `vocs build` (one reason `build` is three commands) and inserts the "When to use deployoor" and "Developer resources" sections. `src/middleware/not-found.ts` is a project middleware — Vocs' server entry globs `src/middleware/*` and appends them to its own chain — that turns the HTML 404 into a Markdown body for clients that did not ask for HTML; `src/pages/404.tsx` overrides Vocs' built-in 404 to carry the same recovery links (a `.tsx` page so it stays out of `llms.txt`, plus `sitemap.include` to keep it out of the sitemap). The identity JSON-LD in `src/lib/site.ts` is rendered by `src/pages/_layout.tsx` rather than passed to Vocs' `head.script`, because Vocs renders `Head` twice per page and dedupes only `title`/`meta`/`link` — through `head.script` every block ships twice. `public/robots.txt` exists to carry pointers to those files, and its presence is what makes Vocs skip generating one. Fifth: `scripts/ping-indexnow.mjs` submits the built sitemap to IndexNow on production builds only (Bing, Yandex, Naver, Seznam and Yep act on it in minutes; Google does not participate). It reads the host and the key location back out of the sitemap so they cannot disagree with the URLs being submitted, and `public/<key>.txt` is a published proof of ownership, not a secret. One thing governs all of it: `siteUrl` in `src/lib/site.ts` is the single canonical host, and it must match the primary domain in Vercel. When it named `www` while `www` redirected to the apex, `<base href>` sent every internal link on every page through a 308 and the canonical pointed at a URL that redirected away from itself.
...
- **Reach for Effect the moment the async work is more than one promise.** Branching on a result, sequencing steps, running work in parallel, racing, retrying, timing out: all of those go through Effect, in any package, not only the engine. A lone `await` needs nothing. What this rules out is the shape that keeps appearing instead — a `reduce` over promises to get sequencing, a `Promise.all` whose failures are then untangled by hand, a chain of `if (result.error) … else if (result.status !== 0)`. `Effect.forEach` with a `concurrency` option says the first two in a line, and `Data.TaggedError` says the third. `evals/` is the worked example outside the engine: one `Effect.runPromise` at the boundary its host demands, Effect everywhere behind it.
- **Tests (Vitest):** third-person `it("does X when Y")` (no "should", no test-case IDs); assert specific errors; for state changes, assert the precondition before and the postcondition after; use `vi.fn()` spies; real-EVM via EDR. Plugin tests inject a mock `fetch` via `PluginDeps`.
