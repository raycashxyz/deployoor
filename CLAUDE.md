...
- **Peer ranges are capped below the next major.** `viem` is `^2` everywhere (viem 3 is imminent; `>=2` would have silently accepted it), `@wagmi/cli` `^2`, `@tevm/compiler` `<2`, `solc` `<0.9`, and the plugin packages' `deployoor` peer is `>=0.5.0 <1.0.0` — not `*`, which accepted any engine version including breaking pre-1.0 minors. The `viem` **version** itself is pinned exactly in `pnpm-workspace.yaml`'s catalog; packages reference it as `catalog:`.
