...
Checked at first use rather than when the plugin is constructed, deliberately. `deployoor.config.ts` is imported by _every_ command, so a construction-time throw made `deployoor generate` exit with an Etherscan credential error over a key it never uses — worse than the problem being fixed, since working locally without an explorer key is the normal case. The check still runs before the first request, so the failure is still local and still names the variable.
