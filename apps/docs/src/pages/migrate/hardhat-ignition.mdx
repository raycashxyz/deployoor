...
await Promise.all(
  Object.entries(addresses).map(([futureId, address]) => {
    ...
    const deploymentName = futureId.split("#").at(-1) as string;
    return register({ publicClient, deploymentName, address, abi });
  }),
);
```
