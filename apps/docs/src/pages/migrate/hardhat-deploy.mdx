...
await Promise.all(
  files.map((file) => {
    ...
    return register({ publicClient, deploymentName: file.replace(/\.json$/, ""), address, abi });
  }),
);
```
...
- **`getOrDeploy` redeploys `on-change` by default** — like hardhat-deploy. Re-running sends no transaction unless the runtime bytecode, constructor args, or linked libraries changed; then it redeploys. Set `redeploymentStrategy: 'never'` to only ever reuse, or `'always'` to redeploy every run. See [idempotency](/concepts/idempotency).
