...
A store is four functions, plus optional ones. They can be sync or async.
...
`listAll` is what a repo-wide command reaches for, since `list` is per-network and the adapter is not required to say which networks it holds. Omit it and [`deployoor verify`](/reference/cli#verify) has to be pointed at one network at a time, by its exact `<chainId>-<slug>` key.
