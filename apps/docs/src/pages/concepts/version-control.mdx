...
If a rule in your `.gitignore` covers deployoor's output, `deployoor generate` says so and offers to remove it — see [upgrading](/guides/upgrading#3-commit-deployers).
...
```json
// eslint.config.json / .oxlintrc.json
