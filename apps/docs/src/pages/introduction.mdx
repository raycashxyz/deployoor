...
- **One source of truth** — real deploys, on mainnet, testnets, or a local node, are recorded as plain JSON you commit; a later run reuses the recorded contract instead of deploying again. Tests use an ephemeral in-memory store, so they never write to that folder.
