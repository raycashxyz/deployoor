...
A `publicClient` is enough, and the contract comes back read-only (`contract.read.*`). Pass a `walletClient` as well to get `contract.write.*` too, and to record that account as the registrant.
