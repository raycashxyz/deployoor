...
Set a project default — globally or [per chain](/guides/configuration#options) — in `deployoor.config`. `force: true` / `force: false` remain as deprecated aliases for `"always"` / `"never"`.
