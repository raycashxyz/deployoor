...
| `importExtension`               | `"auto"`                 | `.js` on generated relative imports ([details](#import-extensions))         |
...
extensionless relative import (`TS2835`), so deployoor emits `./types/Counter.js`. Under `bundler`
(or no tsconfig at all) it emits `./types/Counter`, because that is the idiomatic form and a `.js`
specifier is a resolution failure in setups that do not map it back to `.ts` — webpack without
`resolve.extensionAlias`, ts-jest without a `moduleNameMapper`.
...
`@tsconfig/node22` counts). A project with no tsconfig keeps the extensionless form. Override it
when you need to:
