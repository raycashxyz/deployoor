...
plugin cannot be imported outside a Hardhat run — it throws `HH5: HardhatContext is not created`. When
that happens deployoor falls back to reading the literal `paths.artifacts` out of the file as text, so
the common case still works. A computed path (`join(__dirname, …)`) needs `artifactsPath` set here.
:::
