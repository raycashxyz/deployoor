...
Put a compile in front of your test script. A generated deployer holds only a name and an abi, and reads bytecode from your artifacts when you call it, so a test run against an uncompiled tree fails — clearly, but it fails:
