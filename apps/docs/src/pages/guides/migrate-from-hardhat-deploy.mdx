...
Records created this way have `kind: "external"` and no compiler metadata, and `register` runs no plugins at all — so nothing tries to verify them. They were verified by your old tooling already. Contracts you deploy _with_ deployoor from here on carry full metadata, and their verifier plugins can retry on a later re-run.
:::
