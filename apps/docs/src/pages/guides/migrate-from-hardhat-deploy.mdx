...
// Import the SAME viem chain your deploy script uses, and swap it per network. deployoor takes
// both the id and the name from this chain to build the record path
// (`deployments/<chainId>-<network>/`), so a different chain object writes to a different folder
// than your next real deploy — and a client with no `chain` at all rejects with `NoChainOnClient`.
const chain = sepolia;
...
// Guard against importing a mainnet folder as if it were a testnet: hardhat-deploy keeps the
// chain id in a sidecar (.chain in v2, .chainId in v1). Check it, and the live RPC, against the
// chain above before writing anything.
...
const main = async (): Promise<void> => {
  // Fail closed. With no sidecar there is nothing proving which network these records belong to,
  // and guessing would file another chain's addresses under `chain` — silently corrupting the
  // source of truth. hardhat-deploy always writes one, so a missing sidecar means a hand-assembled
  // folder that deserves a look.
  const recorded = sidecarChainId();
  if (recorded === undefined) {
    throw new Error(`${legacyDir} has no .chain or .chainId sidecar — refusing to guess its network`);
  }
  if (recorded !== chain.id) {
    throw new Error(`${legacyDir} records chain ${recorded}, but this script is set to ${chain.id}`);
  }
  const live = await publicClient.getChainId();
  if (live !== chain.id) {
    throw new Error(`RPC_URL is chain ${live}, but this script is set to ${chain.id}`);
  }
  ...
  console.log(`Registered ${imported.length} contract(s) on ${chain.name} (${chain.id}):`);
