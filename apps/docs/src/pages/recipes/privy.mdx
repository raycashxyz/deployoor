...
- On the basic flow your process holds an app secret and a wallet id, and no private key. A leak still lets someone ask Privy to sign for that wallet, so scope the secret and rotate it like any other credential.
